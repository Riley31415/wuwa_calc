...
import {
  ...
  MV, MV_BASE, DMG_BONUS, AMP, FORTE1, FORTE2, FORTE3, FORTE4,
 ...
 */
const SECTION_OF = {
  [BASE_ATK]: "base", [BONUS_ATK]: "bonus", [FLAT_ATK]: "flat",
  [BASE_HP]: "base", [BONUS_HP]: "bonus", [FLAT_HP]: "flat",
};
...
function rowValues(snap, { mv, avg, config, levels }) {
  ...
  if (raw.mv) {
    sources.mv = [
      { source: snap.action.id, stat: "Base Motion Value", value: snap.action.mv, percent: true },
      ...(sources.mv ?? []),
    ];
  }
  ...
  if (config) {
    ...
    // Every term of the damage product, in the order the formula multiplies them.
    sources.avg = [
...
export function buildReport(lines, { strip = null, config = null, levels = null } = {}) {
  const columns = [
    ...
    // fed them.
    { key: "mv", label: "mv%", width: 10, digits: 2, percent: true },
    { key: "avg", label: "avg dmg", width: 10 },
    { key: "atk", label: "atk", width: 7, noun: "attack" },
    { key: "hp", label: "hp", width: 8, noun: "HP" },
    { key: "dmgBonus", label: "dmg+%", width: 7, digits: 1, percent: true },
    { key: "amp", label: "amp%", width: 6, digits: 1, percent: true },
    { key: "cr", label: "cr%", width: 7, digits: 1, percent: true },
    { key: "cd", label: "cd%", width: 6, digits: 1, percent: true },
    { key: "effDef", label: "def%", width: 6, digits: 1, percent: true },
    { key: "effRes", label: "res%", width: 6, digits: 1, percent: true },

    { key: "energy", label: "energy", width: 7, digits: 1, hideIfZero: true },
    { key: "concerto", label: "concerto", width: 9, digits: 1, hideIfZero: true },
    { key: "offtune", label: "offtune", width: 8, digits: 2, hideIfZero: true },
    ...FORTE_GAUGES.map((key) => ({
      key: `gauge:${key}`, label: key, width: 7, hideIfZero: true,
    })),
  ];
