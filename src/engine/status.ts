 ...
 * does read is the *acting resonator's* own — Hiyuki's Fine Snow, Frostburn's Self No More. Also
 * like a Tune Break, it is an ordinary active cast that merely reports under `MISC_SLOT` (declared
 * on the action below), keeping the target's own damage out of that resonator's column. Active
 * because the resonator really is on field for it: marking it otherwise would have every "lost on
 * switching out" buff on the team drop the moment a rung ticked. It is `triggered` (queued, never
...
import {
  ...
  addEnemyStat, applied, appliedByMe, applyEnemy, currentAction, currentTeam, queue, removeStackEnemy, revokeSelf, revokeEnemy,
...
const negativeStatusActions = (name: string, element: Attribute, type2: Type2, mvs: number[]): (Action | null)[] =>
  [null, ...mvs.map((mv, i) => new Action(`${name} - ${i + 1} Stack${(i+1)>1 ? "s" : ""}`, {
    element, type: Type1.Status, type2, scaling: Scaling.Dot, mv, slot: MISC_SLOT,
  }))];
