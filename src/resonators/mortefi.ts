...
export const Skill = mortefiAction("Skill - Passionate Variation", { node: Node.Skill, cast: Cast.Skill, type: Type1.Skill, mv: 208.76, energy: 10, concerto: 18, offtune: 7200, forte1: 40 });

// --- forte circuit: Fury Fugue — spends every point of Annoyance, considered Resonance Skill DMG
export const FSkill = mortefiAction("Forte Skill - Fury Fugue", { node: Node.Forte, cast: Cast.Skill, type: Type1.Skill, mv: 326.05, energy: 10, concerto: 18, offtune: 8000, forte1: -100 });
...
export const ACTION_S5_MARCATO = mortefiAction("Liberation - Marcato x4 (S5 Funerary Quartet)", { node: Node.Liberation, type: Type1.Liberation, type2: Type2.Coordinated, mv: 31.81 * 4 * 0.5 });

// --- intro / outro
export const Intro = mortefiAction("Intro - Dissonance", { node: Node.Intro, cast: Cast.Intro, type: Type1.Intro, mv: 168.99, energy: 10, concerto: 10, offtune: 8000 });
export const Outro = mortefiAction("Outro - Rage Transposition", { cast: Cast.Outro, mv: 0, active: false });
...
export const BURNING_RHAPSODY = new Buff({
  name: "Mortefi: Burning Rhapsody",
  update: () => { if (casting(Cast.Outro) && isHeld(MORTEFI)) revokeTeam(BURNING_RHAPSODY); },
});
...
export const HARMONIC_CONTROL = new Buff({
  name: "Mortefi: Harmonic Control",
  apply: () => { if (currentAction() === FSkill) addStat(Stat.DmgBonus, 25); },
  convert: () => { if (casting(Cast.Outro)) revoke(HARMONIC_CONTROL); },
});
/** Harmonic Control's own trigger — always-equipped Inherent Skill piece. */
export const MO_INHERENT_1 = new Inherent({
  name: "Mortefi: Harmonic Control",
  update: () => { if (currentAction() === Skill) applySelf(HARMONIC_CONTROL, 1); },
});
...
export const MO_INHERENT_2 = new Inherent({ name: "Mortefi: Rhythmic Vibrato" });
...
export const MORTEFI_OUTRO = new Buff({
  name: "Mortefi: Outro",
  apply: () => addStat(Stat.Amp, 38, Type1.Heavy),
  update: () => { lostOnSwap(); },
});

/** S6 Apoplectic Instrumental: team-wide +20% ATK, 20s — lost on the applier's own next Intro. */
export const S6_TEAM_ATK = new Buff({
  name: "Mortefi S6: Apoplectic Instrumental",
  apply: () => addStat(Stat.BonusAtk, 20),
  convert: () => { if (casting(Cast.Intro) && isHeld(MORTEFI)) revokeTeam(S6_TEAM_ATK); },
});
...
export const MORTEFI_S1 = new Sequence({ name: "Mortefi S1: Solitary Etude" });

/** S2 Hypocritical Hymn: +10 Energy on Echo Skill — its own 20s ICD dropped, per the standing
 *  ICD simplification. */
export const MORTEFI_S2 = new Sequence({
  name: "Mortefi S2: Hypocritical Hymn",
  update: () => { if (casting(Cast.Echo)) addStat(Stat.AddEnergy, 10); },
});
...
export const MORTEFI_S3 = new Sequence({
  name: "Mortefi S3: Flaming Recitativo",
  apply: () => addStat(Stat.CritDmg, 30, Type2.Coordinated),
});
...
export const MORTEFI_S4 = new Sequence({ name: "Mortefi S4: Cathartic Waltz" });
...
export const MORTEFI_S5 = new Sequence({
...
export const MORTEFI_S6 = new Sequence({
  name: "Mortefi S6: Apoplectic Instrumental",
  update: () => { if (currentAction() === Liberation) applyTeam(S6_TEAM_ATK, 1); },
});
...
export const MORTEFI_TALENTS = new Talent({
