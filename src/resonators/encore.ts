 ...
 *  S1 a landed Basic Attack grants +3% Fusion DMG Bonus, up to 4 stacks, 6s.
...
export const BA1 = encoreAction("Basic - Wooly Attack 1", { node: Node.Normal, cast: Cast.Basic, type: Type1.Basic, mv: 55.66, energy: 0.70, concerto: 1.40, offtune: 3360, forte1: 3 });
export const BA2 = encoreAction("Basic - Wooly Attack 2", { node: Node.Normal, cast: Cast.Basic, type: Type1.Basic, mv: 66.20, energy: 0.83, concerto: 1.66, offtune: 3996, forte1: 5 });
export const BA3 = encoreAction("Basic - Wooly Attack 3", { node: Node.Normal, cast: Cast.Basic, type: Type1.Basic, mv: 132.60, energy: 1.66, concerto: 3.32, offtune: 8004, forte1: 6 });
export const BA4 = encoreAction("Basic - Wooly Attack 4", { node: Node.Normal, cast: Cast.Basic, type: Type1.Basic, mv: 153.08, energy: 1.92, concerto: 3.84, offtune: 9240, forte1: 4 });
...
export const UBA1 = encoreAction("Basic - Cosmos: Frolicking 1", { node: Node.Liberation, cast: Cast.Basic, type: Type1.Basic, mv: 180.36, energy: 1.32, concerto: 2.66, offtune: 6396, forte1: 8 });
export const UBA2 = encoreAction("Basic - Cosmos: Frolicking 2", { node: Node.Liberation, cast: Cast.Basic, type: Type1.Basic, mv: 169.20, energy: 1.23, concerto: 2.49, offtune: 6000, forte1: 12 });
export const UBA3 = encoreAction("Basic - Cosmos: Frolicking 3", { node: Node.Liberation, cast: Cast.Basic, type: Type1.Basic, mv: 263.96, energy: 1.92, concerto: 3.88, offtune: 9360, forte1: 16 });
export const UBA4 = encoreAction("Basic - Cosmos: Frolicking 4", { node: Node.Liberation, cast: Cast.Basic, type: Type1.Basic, mv: 582.03, energy: 4.29, concerto: 8.58, offtune: 20640, forte1: 27 });
...
export const Outro = encoreAction("Outro - Thermal Field", { cast: Cast.Outro, type: Type1.Outro, mv: 707.04, active: false });

/* ------------------------------------------------------------------------------------ buffs */

/** +10% Fusion DMG Bonus for 10s on casting Flaming Woolies or Cosmos - Rampage. */
export const WOOLIES_CHEER_DANCE = new Buff({
  name: "Encore: Woolies Cheer Dance",
  apply: () => addStat(Stat.DmgBonus, 10, Attribute.Fusion),
  convert: () => { if (casting(Cast.Outro)) revoke(WOOLIES_CHEER_DANCE); },
});
export const EN_INHERENT_2 = new Inherent({
  name: "Encore: Woolies Cheer Dance",
  update: () => { const a = currentAction(); if (a === Skill1 || a === USkill) applySelf(WOOLIES_CHEER_DANCE, 1); },
});

/** Assumed always true — see file header. Granted/revoked alongside Cosmos Rave itself, so
 *  apply() doesn't need to check any particular action. */
export const ANGRY_COSMOS = new Buff({
  name: "Encore: Angry Cosmos",
  apply: () => addStat(Stat.DmgBonus, 10),
  convert: () => { if (currentAction() === FHA) revoke(ANGRY_COSMOS); },
});
export const EN_INHERENT_1 = new Inherent({
  name: "Encore: Angry Cosmos",
  update: () => { if (currentAction() === Liberation) applySelf(ANGRY_COSMOS, 1); },
});

/* ------------------------------------------------------------------------------- sequences */

export const S1_STACKS = new Buff({
  name: "Encore S1: Wooly's Fairy Tale", maxStacks: 4,
  apply: () => addStat(Stat.DmgBonus, 3 * stacks(), Attribute.Fusion),
  convert: () => { if (casting(Cast.Outro)) revoke(S1_STACKS); },
});
export const S1 = new Sequence({
  name: "Encore S1",
  update: () => { if (casting(Cast.Basic)) applySelf(S1_STACKS, 1); },
});

// 10s ICD isn't modelled, so it pays every cast instead of once per window
export const S2 = new Sequence({
  name: "Encore S2", // note removed ba5 trigger to model 10s cooldown
  update: () => { if (currentAction() === Skill2) addStat(Stat.AddEnergy, 10); },
});

export const S3 = new Sequence({
  name: "Encore S3",
  apply: () => { if (currentAction() === CloudyFrenzy || currentAction() === FHA) addStat(Stat.MulMv, 40); },
});
...
export const S4_TEAM = new Buff({
  ...
  apply: () => addStat(Stat.DmgBonus, 20, Attribute.Fusion),
});
export const S4 = new Sequence({
  name: "Encore S4",
  update: () => { if (currentAction() === FHA) applyTeam(S4_TEAM, 1); },
});

export const S5 = new Sequence({
  name: "Encore S5",
  apply: () => addStat(Stat.DmgBonus, 35, Type1.Skill),
});

export const S6_LOST_LAMB = new Buff({
  name: "Encore S6: Lost Lamb", maxStacks: 5,
  apply: () => addStat(Stat.BonusAtk, 5 * stacks()),
  convert: () => { if (casting(Cast.Outro)) revoke(S6_LOST_LAMB); },
});
export const S6 = new Sequence({
  name: "Encore S6",
  update: () => { if (isHeld(WOOLIES_CHEER_DANCE)) applySelf(S6_LOST_LAMB, 1); },
});
...
export const ENCORE_TALENTS = new Talent({
  name: "Encore: Talents",
  apply: () => { addStat(Stat.BonusAtk, 12); addStat(Stat.DmgBonus, 12, Attribute.Fusion); },
});
