 ...
 * variants, and the first kit to use them (see kit.ts's own `State.shifting`/`TUNE_VARIANTS`).
 *
...
import {
  ...
  addStat, applySelf, applyTeam, casting, currentAction, queue, queueOutro, revoke, shift,
  stacksOfEnemy, TUNE_STRAIN_INTERFERED, TUNE_BREAK_RUPTURE,
} from "../kit.js";
import { SPECTRUM_BLASTER } from "../weapons/pistol.js";
import { NEW_STD_PISTOL } from "../weapons/standard.js";
import { HYVATIA } from "../echoes/lahairoi.js";
...
/** Her answer to a Rupture break — queued by the engine's own break rather than played (see
 *  LYNAE's own update()), so it lands whether or not she is the one on field. `active: false` for
 *  that reason, same as every other off-field follow-up in this project. */
export const SpectralAnalysis = lynaeAction("Tune Rupture Response - Spectral Analysis", {
  node: Node.Forte, type: Type1.Rupture, mv: 1880.75, active: false,
});
 ...
 *  and the engine's own exclusivity rule (one Shifting at a time) does the rest. */
export const MODE_RUPTURE = new ResonanceMode({
  name: "Lynae: Resonance Mode - Tune Rupture",
  update: () => { if (inflictsFlux(currentAction())) shift(Type1.Rupture); },
});
export const MODE_STRAIN = new ResonanceMode({
  name: "Lynae: Resonance Mode - Tune Strain",
  update: () => { if (inflictsFlux(currentAction())) shift(Type1.Strain); },
});
...
export const PRISMATIC_OVERBLAST = new Buff({
  ...
  apply: () => { if (currentAction().active) addStat(Stat.DmgBonus, 24); },
});
...
export const LYNAE_OUTRO = new Buff({
  ...
  convert: () => { if (casting(Cast.Outro)) revoke(LYNAE_OUTRO); },
});
...
export const SPECTRAL_ANALYSIS_TBB = new Buff({
  name: "Lynae: Spectral Analysis (Tune Break Boost)",
  apply: () => addStat(Stat.Tbb, 40),
});

/** Spectral Analysis, the Strain half: every point of her own Tune Break Boost is +0.12% of her
 *  total damage per stack of Tune Strain - Interfered on the target. The stacks are read live off
 *  the enemy (kit.ts's `TUNE_STRAIN_INTERFERED`, applied by the engine as each Strain break
 *  resolves) rather than assumed, so it ramps with the breaks the rotation actually lands.
 *
 *  Her Tune Break Boost is written out rather than read back through `get(Stat.Tbb)`: this runs
 *  during the same apply phase that contributes it, so the running total isn't final yet. 10 from
 *  the resonator plus 40 from the kit — a Tbb main stat would need adding here too. */
const LYNAE_TBB = 10 + 40;
export const SPECTRAL_ANALYSIS_STRAIN = new Buff({
  name: "Lynae: Spectral Analysis (Strain)",
  apply: () => {
    const interfered = stacksOfEnemy(TUNE_STRAIN_INTERFERED);
    if (interfered > 0) addStat(Stat.TotalDmg, 0.12 * LYNAE_TBB * interfered);
  },
});

/** Responding to Tune Rupture - Interfered: when the team's Tune Break resolves as a Rupture, she
 *  answers with Spectral Analysis. `updateGlobal` so it sees the break whoever is on field — the
 *  engine runs a locally-held gear's own updateGlobal with `currentSlot` switched to its holder,
 *  which is also what pins the queued follow-up to her rather than to the actor (kit.ts's own
 *  `queue()`). The skill's own once-per-8s-per-target limit never binds: a rotation lands about
 *  one break a loop, nowhere near that often. */
export const SPECTRAL_ANALYSIS_RUPTURE = new Buff({
  name: "Lynae: Spectral Analysis (Rupture)",
  updateGlobal: () => { if (currentAction() === TUNE_BREAK_RUPTURE) queue(SpectralAnalysis); },
});
...
export const LYNAE = new Resonator({
  ...
  update: () => {
    ...
    // both halves of Spectral Analysis are always on — the Strain one pays nothing until the
    // target is actually Interfered, so holding it costs the report a row and nothing else
    applySelf(SPECTRAL_ANALYSIS_TBB, 1);
    applySelf(SPECTRAL_ANALYSIS_STRAIN, 1);
    applySelf(SPECTRAL_ANALYSIS_RUPTURE, 1);
  },
...
export const LY_ROTATION = [
  INTRO, SparkCollision, Skill,
  PolychromeLeap1, PolychromeLeap2, PolychromeLeap3,
  VisualImpact, IridescentSplash,
  ECHO_CAST, Liberation, VividTomorrow,
  KBA1, KBA2, KBA3, KBA4, KBA5,
  Outro,
];

const LY_ECHOES = [
  new EchoLoadout(HYVATIA, MOONLIT_CLOUDS_5PC, MOONLIT_CLOUDS_2PC),
...
const build = (mode: ResonanceMode): Loadout => new Loadout(
  ...
  [SPECTRUM_BLASTER, NEW_STD_PISTOL],
  LY_ECHOES,
