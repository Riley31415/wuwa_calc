...
const PH_INHERENT_1 = new Inherent({
  name: "Phrolova: Accidental",
  updateBuffs: () => { const a = currentAction(); if (a === Intro || a === EIntro || casting(Cast.Echo)) applySelf(ACCIDENTAL, 1); },
});
...
const PH_S2 = new Sequence({
  ...
  updateBuffs: () => { if (currentAction() === ScarletCoda) applySelf(AFTERSOUND, 14); },
