...
const IUNO_DOMAIN = new Buff({
  ...
  updateBuffs: () => { if (applied(SHIELD)) applySelf(IUNO_BLESSING, applied(SHIELD)); },
});

const IO_INHERENT_2 = new Inherent({
  name: "Iuno: Derivation",
  updateBuffs: () => { if (casting(Cast.Intro) || casting(Cast.Liberation)) applySelf(IUNO_BLESSING, 5); },
});
