...
import {
  ...
  Type2, Cast, Node, Scaling, applySelf, currentAction, casting, queue, queueOutro, removeStackTeam, revokeSelf,
...
const EHA = cantaAction("Heavy - Delusive Dive", {
  ...
  updateBuffs: () => applySelf(MIRAGE, 1),
});
...
const ESkill = cantaAction("Skill - Flickering Reverie", {
  ...
  updateBuffs: () => applySelf(HAZY_DREAM, 1),
});
...
const POISON = new Buff({
  ...
  convertStats: () => { if (casting(Cast.Outro)) revokeSelf(POISON); },
});
...
const MIRAGE = new Buff({
  ...
  updateBuffs: () => { if (forte1() <= 0 || casting(Cast.Outro)) revokeSelf(MIRAGE); },
});
...
const CANTARELLA = new Resonator({
  ...
  updateDebuffs: () => {
    ...
    if (a === FBA1 || a === FBA2 || a === FBA3 || a === FSkill) applySelf(HEALS, 1);
  },
...
const CA_ROTATION = new Rotation([
  ...
  Liberation, EHA, FBA1, ESkill, FBA2, FBA3, FSkill, OUTRO_NEXT,
]);
