...
import { isType,
  ...
  removeStackTeam, revoke, addStat, frozenStacks,
...
const EHA = cantaAction("Heavy - Delusive Dive", { node: Node.Normal, cast: Cast.Heavy, type: Type1.Heavy, mv: 106.1, energy: 1.68, concerto: 3.34, offtune: 5336 }); // 53.05%x2

const FBA1 = cantaAction("Forte - Phantom Sting 1", { node: Node.Forte, cast: Cast.Basic, type: Type1.Basic, mv: 105.99, energy: 1.35, concerto: 2.67, offtune: 4266, forte1: -1, forte2: 1, heals: true }); // 35.33%x3
const FBA2 = cantaAction("Forte - Phantom Sting 2", { node: Node.Forte, cast: Cast.Basic, type: Type1.Basic, mv: 125.86, energy: 1.6, concerto: 3.18, offtune: 5064, forte1: -1, forte2: 1, heals: true }); // 62.93%x2
const FBA3 = cantaAction("Forte - Phantom Sting 3", { node: Node.Forte, cast: Cast.Basic, type: Type1.Basic, type2: Type2.Coordinated, mv: 258.48, energy: 3.28, concerto: 6.52, offtune: 10400, forte1: -1, forte2: 1, heals: true }); // 64.62%x4
...
const ESkill = cantaAction("Skill - Flickering Reverie", { node: Node.Skill, cast: Cast.Skill, cast2: Cast.Echo, type: Type1.Skill, mv: 196.23, energy: 1.65, concerto: 10, offtune: 5264 });
/** At 3 Shiver — CANTARELLA's own updateBuffs() resets forte2 to 3 first, so -3 lands on 0 exactly. */
const FSkill = cantaAction("Forte - Perception Drain", { node: Node.Forte, cast: Cast.Skill, cast2: Cast.Echo, type: Type1.Basic, mv: 1335.98, energy: 21.1, concerto: 12, offtune: 57864, forte2: -3, heals: true }); // 667.99%x2
...
const Intro = cantaAction("Intro - Ripple", { node: Node.Intro, cast: Cast.Intro, type: Type1.Intro, mv: 169, energy: 3.16, concerto: 10, offtune: 10120, forte1: 1 }); // 42.25%x4
const Outro = cantaAction("Outro - Gentle Tentacles", { cast: Cast.Outro, mv: 0, active: false });
...
const POISON = new Buff({
  ...
  convertStats: () => { if (casting(Cast.Outro)) revoke(POISON); },
});
...
const MIRAGE = new Buff({
  ...
  updateBuffs: () => { if (forte1() <= 0 || casting(Cast.Outro)) revoke(MIRAGE); },
});
...
const CA_INHERENT_2 = new Inherent({
  ...
  updateBuffs: () => { if (casting(Cast.Echo)) applySelf(POISON, 1); },
});
...
export const CANTA_LOADOUT = new Loadout({
  ...
  weapons: [WHISPERS_OF_SIRENS, NEW_STD_RECTIFIER, COSMIC_RIPPLES],
  echoLoadouts: [
