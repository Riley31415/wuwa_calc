...
import {
  ...
  Cast, Node, Scaling, applyCurrent, applyTeam, revokeTeam, isHeld, revokeSelf, casting, currentAction, addStat,
...
import { ELECTRO_FLARE, inflictedNegativeStatus, HEALS } from "../../engine/status.js";
...
const ELECTRO_CORE = new Buff({
  ...
  updateBuffs: () => {
    lostOnSwap();
    if (inflictedNegativeStatus()) { applyCurrent(ER_OUTRO, 1); revokeSelf(ELECTRO_CORE); }
  },
});
