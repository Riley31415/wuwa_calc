...
export const SH_INHERENT_1 = new Inherent({
  name: "Sanhua: Condensation",
  update: () => { if (currentAction() === Intro) applySelf(CONDENSATION, 1); },
});
...
export const SH_INHERENT_2 = new Inherent({
  name: "Sanhua: Avalanche",
  update: () => { if (currentAction() === BA5) applySelf(AVALANCHE, 1); },
});
 ...
 *  Detonate's own update() below, which queues the matching burst(s). No stat of their own. */
...
// trigger a sequence needs lives in its own piece, not the central Resonator update() below.

export const SANHUA_S1 = new Sequence({
  name: "Sanhua S1: Solitude's Embrace",
...
export const SANHUA_S2 = new Sequence({ name: "Sanhua S2: Snowy Clarity" });

// S3 Anomalous Vision: flat DMG Bonus, no separate trigger needed
export const SANHUA_S3 = new Sequence({
  name: "Sanhua S3: Anomalous Vision", apply: () => addStat(Stat.DmgBonus, 24.5),
});

export const SANHUA_S4 = new Sequence({
  name: "Sanhua S4: Blade Mastery",
...
export const SANHUA_S5 = new Sequence({
...
export const SANHUA_S6 = new Sequence({
...
export const SANHUA = new Resonator({
  ...
  // grants each Ice Creation marker off the cast that makes it, then Detonate's own update()
...
export const SANHUA_TALENTS = new Talent({
  name: "Sanhua: Talents",
  apply: () => { addStat(Stat.BonusAtk, 12); addStat(Stat.DmgBonus, 12, Attribute.Glacio); },
});
