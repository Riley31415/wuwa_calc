...
import { HERON, MOONLIT_CLOUDS_5PC, MOONLIT_CLOUDS_2PC, LINGERING_TUNES_2PC, VOID_THUNDER_2PC } from "../../echoes/jinzhou.js";
...
// the old mode
const TO_GUTS = { convertStats: () => { revokeSelf(HUNTRESS); applyCurrent(GUTS, 1); } };
const TO_HUNTRESS = { convertStats: () => { revokeSelf(GUTS); applyCurrent(HUNTRESS, 1); } };
...
const Lib3 = rebeccaAction("Liberation - Mk. 31 HMG 1st Enhancement x5", {
  node: Node.Liberation, type: Type1.Basic, mv: 48.6 * 5, concerto: 1.12 * 5, offtune: 3218 * 5,
});
const Lib4 = rebeccaAction("Liberation - Mk. 31 HMG 2nd Enhancement x10", {
  node: Node.Liberation, type: Type1.Basic, mv: 72.9 * 10, concerto: 1.67 * 10, offtune: 4826 * 10,
});
