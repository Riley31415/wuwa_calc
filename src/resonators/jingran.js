...
import {
  add, hp, spend, counter, setCounter, action, grantTeam,
  equipped, grantSelf, slotsWith, queue, note,
  addStack, stacksOf, removeStack, revoke,
} from "../state.js";
import {
  BASE_ATK, BASE_HP, BONUS_HP, FLAT_ATK, CRIT_RATE, CRIT_DMG, ER, ADD_MV, DEF_IGNORE,
  DMG_BONUS,
  FUSION, BASIC, HEAVY, SKILL, INTRO, OUTRO, ECHO, LIB, NORMAL, FORTE,
  SCALE_HP,
  FORTE1, FORTE2,
  FLAT_DEF,
} from "../stats.js";
 ...
 *               both are in every rotation. While it holds, each damaging cast shields him,
 *               which is the EARTH_CHARM response below rather than a field on every action.
 *
...
export const JINGRAN_GHOST_SHROUD = new Buff(PRIORITY.BUFF_STATS,
  () => `Jingran: Ghost Shroud x${stacksOf(JINGRAN_GHOST_SHROUD)}`, 50);
 ...
 */
export const JINGRAN_FORTUNE = new Buff(PRIORITY.EARLY_CONVERSION, () => {
  ...
  if (held) {
    // 0.05% fusion per 1000 Max HP per stack, capped at 2.5% a stack — which is exactly
    // where the 50,000 HP ceiling on his conversions comes from
    const perStack = Math.min(2.5, 0.05 * Math.floor(hp() / 1000));
    add(perStack * held, FUSION, DMG_BONUS);
  }
  return `Jingran: Fortune in Disguise x${held}`;
}, 50);
 ...
 */
export const JINGRAN_GHOST_FEED = new Buff(PRIORITY.UPDATE_BUFFS, () => {
  const shields = action().shields;
  if (shields) {
    // The kit states it twice: the base rule pays 1 whenever anyone on the team is shielded,
    // and Trace the Vestige pays 2 for a resonator *other than* Jingran — so his own Earth
    // Charm shields are worth half what a teammate's are.
...
export const JINGRAN = new Gear(() => {
  ...
  // He does not walk in empty: entering combat tops Ghost Shroud up to 25, the inherent
  // skill rather than an assumption about a previous rotation, so his opener converts at least
  // 25 stacks of Fortune in Disguise even with nobody shielding him.
  addStack(JINGRAN_GHOST_SHROUD, 25);
}, FUSION);
...
export const JINGRAN_SIG = new Gear(() => {
  add(413, BASE_ATK);
  add(72.2, BONUS_HP);
  add(12, DMG_BONUS);
  ...
  const a = action();
  const gained = a.shields + (a.cast === INTRO ? 1 : 0);
  if (gained) {
...
export const NATURES_ORDER = new Buff(PRIORITY.BUFF_STATS, () => {
  const held = stacksOf(NATURES_ORDER);
  add(6 * held, CRIT_DMG);
  // the full six pay crit rate, scoped so only heavy attack damage resolves it
  if (held >= 6) add(12, HEAVY, CRIT_RATE);
  return `Thousandfold Deliverance: Nature's Order x${held}`;
}, 6);
 ...
 *
 * "A heavy attack" is the cast, not the damage type — his basic stages 3 and 4 deal Heavy Attack
 * DMG and must not spend this. That distinction used to be a regex over the action's own name,
 * which quietly stopped matching whenever an action was renamed.
 */
export const CRADLE_OF_LIFE = new Buff(PRIORITY.BUFF_STATS, () => {
  const held = stacksOf(CRADLE_OF_LIFE);
  // the name reports what it is holding *now*, before a heavy attack spends any of it
  const name = `Thousandfold Deliverance: Cradle of Life x${held}`;
  if (!held || action().cast !== HEAVY) return name;

  const spent = Math.min(held, 2);
  add(15 * spent, HEAVY, DEF_IGNORE);
 ...
 */
export const LAMP_5PC = new Gear(() => {
  const { shields } = action();
  if (shields) addStack(LAMP_STACKS, shields);
  return "Lamp of Nether Road 5pc";
});

export const LAMP_STACKS = new Buff(PRIORITY.BUFF_STATS, () => {
  const held = stacksOf(LAMP_STACKS);
  add(5 * held, CRIT_RATE);
  if (held >= 4) add(15, FUSION, DMG_BONUS);
  return `Lamp of Nether Road x${held}`;
}, 4);

export const LAMP_2PC = new Gear(() => { add(10, BONUS_HP); return "Lamp of Nether Road 2pc"; });
...
function heavyAttack(mvPer1000) {
  ...
  if (mingfire > 25) {
    setCounter(JINGRAN_QI, Math.min(300, counter(JINGRAN_QI) + 200));
  }
...
const BA1 = jingranAction("Yang Basic 1", { node: NORMAL, cast: BASIC, shields: 1, type: BASIC, mv: 39.82, energy: 0.67, concerto: 1.34, offtune: 0.2136 });
const BA2 = jingranAction("Yang Basic 2", { node: NORMAL, cast: BASIC, shields: 1, type: BASIC, mv: 99.47, energy: 1.68, concerto: 3.35, offtune: 0.5337 });
const BA3 = jingranAction("Yang Basic 3", { node: NORMAL, cast: BASIC, shields: 1, type: HEAVY, mv: 159.1, energy: 2.69, concerto: 5.36, offtune: 0.8537, forte1: 50 });
const BA4 = jingranAction("Yang Basic 4", { node: NORMAL, cast: BASIC, shields: 1, type: HEAVY, mv: 124.24, energy: 2.09, concerto: 4.18, offtune: 0.6666, forte1: 50 });
const MA = jingranAction("Plunge", { node: NORMAL, cast: BASIC, shields: 1, type: BASIC, mv: 92.45, energy: 1.55, concerto: 3.1, offtune: 0.496 });

// --- enhanced basics
const EBA1 = jingranAction("Yin Basic 1", { node: NORMAL, cast: BASIC, shields: 1, type: BASIC, mv: 44.74, energy: 0.75, concerto: 1.5, offtune: 0.24 });
const EBA2 = jingranAction("Yin Basic 2", { node: NORMAL, cast: BASIC, shields: 1, type: BASIC, mv: 74.56, energy: 1.26, concerto: 2.5, offtune: 0.4 });
const EBA3 = jingranAction("Yin Basic 3", { node: NORMAL, cast: BASIC, shields: 1, type: HEAVY, mv: 109.32, energy: 1.84, concerto: 3.68, offtune: 0.5864, forte1: 50 });
const EBA4 = jingranAction("Yin Basic 4", { node: NORMAL, cast: BASIC, shields: 1, type: HEAVY, mv: 153.16, energy: 2.6, concerto: 5.16, offtune: 0.8218, forte1: 50 });
...
const DC = jingranAction("Yang Dodge Counter", { node: NORMAL, cast: BASIC, shields: 1, type: HEAVY, mv: 198.8, energy: 3.36, concerto: 6.68, offtune: 1.0664, forte1: 100 });
const EDC = jingranAction("Yin Dodge Counter", { node: NORMAL, cast: BASIC, shields: 1, type: HEAVY, mv: 248.57, energy: 4.19, concerto: 8.36, offtune: 1.3337, forte1: 100 });
...
const Skill1 = jingranAction("Yang Skill 1", {
  node: SKILL, cast: SKILL, shields: 1, type: SKILL, mv: 164.04, energy: 1.75, concerto: 3.5, offtune: 0.56
});
const ESkill1 = jingranAction("Yin Skill 1", {
  node: SKILL, cast: SKILL, shields: 1, type: SKILL, mv: 164.04, energy: 1.75, concerto: 3.5, offtune: 0.56
});
const Skill2 = jingranAction("Yang Skill 2", {
  node: SKILL, cast: SKILL, shields: 1, type: HEAVY, mv: 258.47, energy: 3.35, concerto: 5, offtune: 1.0667,
  forte1: 100
});
const ESkill2 = jingranAction("Yin Skill 2", {
  node: SKILL, cast: SKILL, shields: 1, type: HEAVY, mv: 263.48, energy: 3.43, concerto: 5, offtune: 1.0936,
  forte1: 100
});
...
const Lib = jingranAction("Liberation", {
  node: LIB, cast: LIB, shields: 2, type: HEAVY, mv: 745.2,      // 93.15% x 8
...
/** Chimei Wangliang — the Yinghuo follow-up, one per heavy attack. It comes out of the
 *  liberation (`node: LIB`, so its damage is attributed there) without being a cast of its own:
 *  no `cast`, so it fires no trigger and earns no Earth Charm shield. */
export const ACTION_LIB_FUA = jingranAction("Liberation FUA", {
  node: LIB,
  type: HEAVY,
  mv: 83.51,
});
...
const Intro = jingranAction("Intro", {
  node: INTRO, cast: INTRO, shields: 1, type: INTRO, mv: 198.81,
...
const Outro = jingranAction("Outro", {
  cast: OUTRO, type: OUTRO, mv: 795, concerto: -100,
...
const FHA = jingranAction("Yang Forte Heavy", {
  node: FORTE, cast: HEAVY, shields: 2, type: HEAVY, mv: 240.38,          // 24.04% + 24.04% + 48.08% + 144.22%
  energy: 8.5, concerto: 13, offtune: 1.04, priority: PRIORITY.LATE_CONVERSION, apply: () => heavyAttack(21.65),   // 2.17% + 2.17% + 4.33% + 12.98%
});
const EFHA = jingranAction("Yin Forte Heavy", {
  node: FORTE, cast: HEAVY, shields: 2, type: HEAVY, mv: 234.29,          // 16.40% x2 + 21.09% x3 + 138.22%
  energy: 8.53, concerto: 13, offtune: 1.014, priority: PRIORITY.LATE_CONVERSION, apply: () => heavyAttack(21.10)    // 1.48% x2 + 1.90% x3 + 12.44%
});
