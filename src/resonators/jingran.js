...
import {
  add, hp, spend, counter, setCounter, gainTeam, grantTeam, action,
...
export const JINGRAN_HP_TO_FUSION = "Jingran: Nether to Light";
export const JINGRAN_HP_TO_ATK = "Jingran: Yang Changes, Yin Unites";
...
export const JINGRAN_GHOST_SHROUD = "Jingran: Ghost Shroud";
export const JINGRAN_FORTUNE      = "Jingran: Fortune in Disguise";
export const JINGRAN_FIXATION     = "Jingran: Fixation";
export const JINGRAN_GHOST_FEED   = "Jingran: Trace the Vestige";
 ...
 */
defineBuff(JINGRAN_GHOST_SHROUD, {
  priority: PRIORITY.BUFF_STATS,
  apply() { return Math.min(stacksOf(JINGRAN_GHOST_SHROUD), SHROUD_MAX); },
});
 ...
 */
defineBuff(JINGRAN_FORTUNE, {
  priority: PRIORITY.EARLY_CONVERSION,
  apply() {
    const held = Math.min(stacksOf(JINGRAN_FORTUNE), FORTUNE_MAX);
...
defineBuff(JINGRAN_FIXATION, { priority: PRIORITY.BUFF_STATS, apply() {} });
...
defineBuff(JINGRAN_GHOST_FEED, {
  ...
  apply() {
    ...
    for (const slot of slotsWith(JINGRAN)) {
      slot.addBuff(JINGRAN_GHOST_SHROUD, { n: shields * per, cap: SHROUD_MAX, via: "state" });
      // Fixation pays once, on a teammate's shield, and is consumed doing it
      if (!own && slot.hasBuff(JINGRAN_FIXATION)) {
        slot.addBuff(JINGRAN_GHOST_SHROUD, { n: FIXATION_SHROUD, cap: SHROUD_MAX, via: "state" });
...
defineGear(JINGRAN, {
  ...
  onFightStart() {
    ...
    addStack(JINGRAN_GHOST_SHROUD, SHROUD_ON_ENTERING_COMBAT, SHROUD_MAX);
  },

  // Pure stats — nothing here reacts to the cast or touches another buff.
  apply() {
 ...
 */
defineBuff(JINGRAN_HP_TO_FUSION, {
  priority: PRIORITY.EARLY_CONVERSION,
  apply() {
    add(-99999, FLAT_DEF)
 ...
 */
export const JINGRAN_SIG_WATCHER = "Thousandfold Deliverance: Call Upon Stars and Ghosts";
export const NATURES_ORDER = "Thousandfold Deliverance: Nature's Order";
...
export const CRADLE_OF_LIFE = "Thousandfold Deliverance: Cradle of Life";
...
defineGear(JINGRAN_SIG, {
  apply() {
    ...
    if (gained) {
      addStack(NATURES_ORDER, gained, NATURES_ORDER_MAX);
      addStack(CRADLE_OF_LIFE, gained, CRADLE_MAX);
    }
  },
});

defineBuff(NATURES_ORDER, {
  priority: PRIORITY.BUFF_STATS,
  apply() {
    const held = Math.min(stacksOf(NATURES_ORDER), NATURES_ORDER_MAX);
 ...
 */
defineBuff(CRADLE_OF_LIFE, {
  priority: PRIORITY.BUFF_STATS,
  apply() {
    const held = Math.min(stacksOf(CRADLE_OF_LIFE), CRADLE_MAX);
 ...
 */
export const LAMP_STACKS = "Lamp of Nether Road:";
const LAMP_MAX = 4, LAMP_CRIT_PER_STACK = 5, LAMP_FULL_FUSION = 15;

defineGear(LAMP_5PC, {
  apply() {
    const { shields } = action();
    if (shields) addStack(LAMP_STACKS, shields, LAMP_MAX);
  },
});

defineBuff(LAMP_STACKS, {
  priority: PRIORITY.BUFF_STATS,
  apply() {
    const held = Math.min(stacksOf(LAMP_STACKS), LAMP_MAX);
 ...
 *
 * Substats are `chem hp heavy` rather than `chem atk`: HP is his real damage stat (it drives
...
export const LOADOUT = [
  ...
  "44111 CD CD hp hp hp", "Chem Substats: hp heavy",
];
...
function heavyAttack(mvPer1000) {
  ...
  if (mingfire > 0) {                                 // the burst window is open
    ...
    //
    // The kit adds this **flat** to the motion value, but MV is a multiplier on the action's
    // own value, so it is converted: base x (1 + MV/100) == base + added.
    const steps = Math.max(0, Math.floor((Math.min(hp(), 50000) - 25000) / 1000));
    add((100 * mvPer1000 * steps) / action().mv, MV);
  }
}
...
jingranAction("BA1", { node: NORMAL, type: BASIC, mv: 39.82, energy: 0.67, concerto: 1.34, offtune: 0.2136, shields: 1 });
jingranAction("BA2", { node: NORMAL, type: BASIC, mv: 99.47, energy: 1.68, concerto: 3.35, offtune: 0.5337, shields: 1 });
jingranAction("BA3", { node: NORMAL, type: HEAVY, mv: 159.1, energy: 2.69, concerto: 5.36, offtune: 0.8537, forte1: 50, shields: 1 });
jingranAction("BA4", { node: NORMAL, type: HEAVY, mv: 124.24, energy: 2.09, concerto: 4.18, offtune: 0.6666, forte1: 50, shields: 1 });
jingranAction("MA", { node: NORMAL, type: BASIC, mv: 92.45, energy: 1.55, concerto: 3.1, offtune: 0.496, shields: 1 });

// --- enhanced basics
jingranAction("EBA1", { node: NORMAL, type: BASIC, mv: 44.74, energy: 0.75, concerto: 1.5, offtune: 0.24, shields: 1 });
jingranAction("EBA2", { node: NORMAL, type: BASIC, mv: 74.56, energy: 1.26, concerto: 2.5, offtune: 0.4, shields: 1 });
jingranAction("EBA3", { node: NORMAL, type: HEAVY, mv: 109.32, energy: 1.84, concerto: 3.68, offtune: 0.5864, forte1: 50, shields: 1 });
jingranAction("EBA4", { node: NORMAL, type: HEAVY, mv: 153.16, energy: 2.6, concerto: 5.16, offtune: 0.8218, forte1: 50, shields: 1 });

// --- dodge counters: Nether Dive / Light Watch, 100 Qi each
jingranAction("DC", { node: NORMAL, type: HEAVY, mv: 198.8, energy: 3.36, concerto: 6.68, offtune: 1.0664, forte1: 100, shields: 1 });
jingranAction("EDC", { node: NORMAL, type: HEAVY, mv: 248.57, energy: 4.19, concerto: 8.36, offtune: 1.3337, forte1: 100, shields: 1 });
...
jingranAction("Intro", {
  ...
  apply() {
    ...
    addStack(JINGRAN_FORTUNE, shroud, FORTUNE_MAX);
  },
});
jingranAction("Outro", {
...
export const BA234 = defineChain("Jingran: BA234",
  ["Jingran: BA2", "Jingran: BA3", "Jingran: BA4"]);
export const EBA234 = defineChain("Jingran: EBA234",
  ["Jingran: EBA2", "Jingran: EBA3", "Jingran: EBA4"]);
...
export const ROTATION = [
  "Jingran: Intro", "Jingran: Lib1", "Jingran: FHA",
  EBA234, "Jingran: EFHA",
  "Jingran: Skill1", "Jingran: Skill2", "Jingran: FHA",
  "Jingran: ESkill1", "Jingran: ESkill2", "Jingran: EFHA",
