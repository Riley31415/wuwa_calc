...
import {
  ...
  Scaling, applyCurrent, revokeSelf, casting, currentAction, addStat, getStat, queueOutro, forte1, setForte1,
  lostOnSwap, applyTeam,
} from "../../engine/kit.js";
import { Rotation, INTRO, OUTRO_NEXT } from "../../engine/rotation.js";
import { SHIELD, HEALS } from "../../engine/status.js";
...
const Liberation = brantAction("Liberation - To the Horizon", {
  ...
  updateBuffs: () => { applyCurrent(AFLAME, 1); revokeSelf(THEATRICAL_MOMENT); applyCurrent(MY_MOMENT, 1); },
});
...
const AFLAME = new Buff({
  ...
  convertStats: () => {
    ...
    revokeSelf(AFLAME);
    revokeSelf(MY_MOMENT); applyCurrent(THEATRICAL_MOMENT, 1);
  },
});
