...
import {
  ...
  Scaling, applyCurrent, forte2, setForte2, stacksOf, isHeld, currentAction, currentTeam, queue, revokeSelf, addStat,
...
const Intro = jingranAction("Intro - Question the Tombs", {
  ...
  updateBuffs: () => {
    ...
    if (shroud) { revokeSelf(JINGRAN_GHOST_SHROUD); applyCurrent(JINGRAN_FORTUNE, shroud); }
  },
});
...
const JR_INHERENT_2 = new Inherent({
  ...
  updateGlobal: () => {
    ...
    if (isHeld(JINGRAN_FIXATION)) { revokeSelf(JINGRAN_FIXATION); applyCurrent(JINGRAN_GHOST_SHROUD, 15); }
  },
});
