...
const SH_INHERENT_1 = new Inherent({
  name: "Sanhua: Condensation",
  updateBuffs: () => { if (currentAction() === Intro) applySelf(CONDENSATION, 1); },
});
...
const SH_INHERENT_2 = new Inherent({
  name: "Sanhua: Avalanche",
  updateBuffs: () => { if (currentAction() === BA5) applySelf(AVALANCHE, 1); },
});
...
const SANHUA_S1 = new Sequence({
  name: "Sanhua S1: Solitude's Embrace",
  updateBuffs: () => { if (currentAction() === BA5) applySelf(S1_CRIT, 1); },
});
...
const SANHUA_S4 = new Sequence({
  name: "Sanhua S4: Blade Mastery",
  updateBuffs: () => { if (currentAction() === Liberation) applySelf(S4_WINDOW, 1); },
});
...
const SANHUA_S5 = new Sequence({
  ...
  updateBuffs: () => { if (currentAction() === Liberation) applySelf(GLACIER_BUFF, 1); },
});
