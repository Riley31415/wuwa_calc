...
import {
  ...
  Type1, Type2, Cast, Node, Scaling, MISC_SLOT, addStat, applied, applyCurrent, applyEnemy,
  applyTeam, currentAction, currentTeam, frozenStacks, queue, queueOn, queueOutro, removeStack,
  removeStackEnemy, revokeEnemy, revokeSelf, revokeTeam, setForte1, setForte2, stacksOf, stacksOfEnemy,
  lostOnSwap,
  forte1,
} from "../../engine/kit.js";
import { Rotation, INTRO, ECHO_CAST, OUTRO_NEXT } from "../../engine/rotation.js";
import { GLACIO_CHAFE, GLACIO_CHAFE_DMG, HAVOC_BANE } from "../../engine/status.js";
...
const FROSTBIND = {
  afterAction: () => { if (stacksOfEnemy(GLACIO_BITE) >= 10) removeStackEnemy(GLACIO_BITE, 10); },
};
...
const Lib2Tap = hiyukiAction("Liberation - Foreclaiming: Blade Liberation", {
  ...
  applyStats: () => {
    if (stacksOf(SNOWFORGED_BLADE) >= 3) {
      addStat(Stat.AddMv, 795.24 * stacksOf(SNOWFORGED_BLADE));
    }
  },
...
const Iai = hiyukiAction("Forte - Basic Attack - Iai", {
  ...
  updateDebuffs: () => { if (stacksOf(FROSTHARDEN_IAI) > 0) applyEnemy(GLACIO_CHAFE, 3); },
  applyStats: () => { if (stacksOf(FROSTHARDEN_IAI) > 0) addStat(Stat.AddForte3, 1); },
  convertStats: () => { if (stacksOf(FROSTHARDEN_IAI) > 0) removeStack(FROSTHARDEN_IAI, 1); },
  ...FROSTBIND,
});
...
const FROSTBLIGHT_ENHANCED = new Buff({
  ...
  convertStats: () => { if (currentAction() === BA3) revokeSelf(FROSTBLIGHT_ENHANCED); },
});
...
const SNOW_RUST = new Buff({
  ...
  // she is the one on field. Both restrictions come free of a condition, from where this sits:
  //
  //  - held locally, so it runs on the acting slot's own turn and no other;
  //  - in `updateDebuffs` rather than `updateBuffs`, which is what makes the count hers alone. The
  //    acting action's own hook runs first in that phase, then local gear, then team gear — so by
  //    here her cast has laid its stacks and Lucilla's Film Roll (team-wide) has not yet added its
  //    two. `appliedByMe` can't stand in: Film Roll re-sources the debuff to Lucilla as it fires,
  //    which would have this read every stack, hers included, as nobody's.
  updateDebuffs: () => {
    if (snowRust() < 2) return;
    for (let i = applied(GLACIO_CHAFE); i > 0; i--) queue(FineSnowBite);
  },
  applyStats: () => {
...
const HY_INHERENT_1 = new Inherent({
  ...
  updateGlobal: () => {
    if (!applied(GLACIO_CHAFE) && !applied(HAVOC_BANE)) return;
