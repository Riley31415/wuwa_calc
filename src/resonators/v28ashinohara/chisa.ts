...
import {
  ...
  Scaling, addStat, applyCurrent, applyTeam, revokeTeam, casting, currentAction, revokeSelf, frozenStacks, applyEnemy,
...
const Outro = chisaAction("Outro - Unraveling - Law Zero", {
  cast: Cast.Outro, type: Type1.Outro, mv: 0, active: false, concerto: -100,
...
const SerratedLoop = chisaAction("Skill - Serrated Loop", { node: Node.Skill, cast: Cast.Skill, type: Type1.Skill, mv: 139.60, energy: 2.96, concerto: 5.92, offtune: 9360, forte1: -100, forte2: 100,...MARK_SNARE });
const SerratedLoopHalfHold = chisaAction("Skill - Serrated Loop (Half Hold)", { node: Node.Skill, cast: Cast.Skill, type: Type1.Skill, mv: 199.28, energy: 4.24, concerto: 8.48, offtune: 13368, forte1: -100,forte2: 100,...MARK_SNARE });
const SerratedLoopHold = chisaAction("Skill - Serrated Loop (Hold)", { node: Node.Skill, cast: Cast.Skill, type: Type1.Skill, mv: 258.96, energy: 5.52, concerto: 11.04, offtune: 17376, forte1: -100, forte2: 100,...MARK_SNARE });
...
const WOVEN_MYRIAD_CONVERGENCE = new Buff({
  ...
  convertStats: () => { if (currentAction() === Eradication) revokeSelf(WOVEN_MYRIAD_CONVERGENCE); },
});
...
const RING_CONSUMED = new Buff({
  ...
  convertStats: () => { if (currentAction() === Eradication) revokeSelf(RING_CONSUMED); },
});
...
const ALL_ENDS_HERE = new Buff({
  ...
  convertStats: () => { if (currentAction() === Outro) revokeSelf(ALL_ENDS_HERE); },
});
