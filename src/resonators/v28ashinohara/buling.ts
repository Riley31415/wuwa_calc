...
import { ELECTRO_FLARE, HEALS } from "../../engine/status.js";
...
import { mainstats, Mainstat } from "../../engine/mainstats.js";
...
const YANG = { updateBuffs: () => {
  ...
  if (isHeld(MINOR_YIN)) { revokeSelf(MINOR_YANG); revokeSelf(MINOR_YIN); applyCurrent(YIN_YANG_BALANCE, 1); }
} };
const YIN = {
  updateDebuffs: () => applyCurrent(HEALS, 1),
  updateBuffs: () => {
    applyCurrent(MINOR_YIN, 1);
    if (isHeld(MINOR_YANG)) { revokeSelf(MINOR_YANG); revokeSelf(MINOR_YIN); applyCurrent(YIN_YANG_BALANCE, 1); }
  },
};
