...
import {
  ...
  applySelf, applyTeam, applyEnemy, revokeEnemy, stacksOfEnemy, maxStackIncrease, isHeld, revoke, casting, currentAction, addStat,
} from "../../kit.js";
import { AERO_EROSION, SPECTRO_FRAZZLE, HAVOC_BANE, FUSION_BURST, GLACIO_CHAFE, ELECTRO_FLARE } from "../../statuses.js";
...
const MA = roverAction("Basic - Mid-air Attack", { node: Node.Normal, cast: Cast.Basic, type: Type1.Basic, mv: 140.76, energy: 0.52, concerto: 9.6, offtune: 9600 });
const DC = roverAction("Basic - Dodge Counter", { node: Node.Normal, cast: Cast.DodgeCounter, type: Type1.Basic, mv: 175.18, energy: 3.74, concerto: 21.95, offtune: 11944, forte1: 10 });
...
const SkyfallSeverance = roverAction("Skill - Skyfall Severance", { node: Node.Skill, cast: Cast.Skill, type: Type1.Skill, mv: 175.26, energy: 2.52, concerto: 5, offtune: 8001 });
...
const Cloudburst1 = roverAction("Forte - Cloudburst Dance 1", { node: Node.Forte, cast: Cast.Basic, type: Type1.Skill, mv: 128.80, energy: 0.92, concerto: 2.93, offtune: 2928, forte1: 25, heals: true });
const Cloudburst2 = roverAction("Forte - Cloudburst Dance 2", { node: Node.Forte, cast: Cast.Basic, type: Type1.Skill, mv: 141.47, energy: 1.01, concerto: 3.22, offtune: 3216, forte1: 25, heals: true });
const UnboundFlow1 = roverAction("Forte Skill - Unbound Flow 1", { node: Node.Forte, cast: Cast.Skill, type: Type1.Skill, mv: 171.50, energy: 10, concerto: 20, offtune: 29850, forte1: -60, heals: true });
const UnboundFlow2 = roverAction("Forte Skill - Unbound Flow 2", { node: Node.Forte, cast: Cast.Skill, type: Type1.Skill, mv: 723.03, energy: 20, concerto: 20, offtune: 28288, forte1: -60, heals: true });

// --- liberation / intro / outro. Storm's Echo hands the whole team Aeolian Realm (see below).
const Liberation = roverAction("Liberation - Omega Storm", { node: Node.Liberation, cast: Cast.Liberation, type: Type1.Liberation, mv: 536.79, concerto: 20, offtune: 48000, resetEnergy: true, heals: true });
...
const Outro = roverAction("Outro - Storm's Echo", { cast: Cast.Outro, active: false });
...
const SAND_IN_THE_STORM = new Buff({
  ...
  convertStats: () => { if (casting(Cast.Outro)) revoke(SAND_IN_THE_STORM); },
});
const AR_INHERENT_1 = new Inherent({
  name: "Aero Rover: Sand in the Storm",
  updateBuffs: () => { if (currentAction() === Intro) applySelf(SAND_IN_THE_STORM, 1); },
});
...
const S4_SKILL_BONUS = new Buff({
  ...
  convertStats: () => { if (casting(Cast.Outro)) revoke(S4_SKILL_BONUS); },
});
...
const ROVER_AERO = new Resonator({
  ...
  // Skyfall Severance strips every other element's Negative Status off the target and pays back a
  // stack of Aero Erosion per stack removed — capped, as always, by the buff system, so Aeolian
  // Realm's own +3 to that cap is what decides how much of a big strip actually lands
  updateDebuffs: () => {
    if (currentAction() !== SkyfallSeverance) return;
    let removed = 0;
...
const AR_ROTATION = [ // TODO needs an opener
  INTRO,
