...
import {
  ...
  Cast, Node, Scaling, applySelf, applyTeam, applyEnemy, revokeEnemy, stacksOfEnemy, maxStackIncrease, isHeld,
  revokeSelf, casting, currentAction, addStat,
} from "../../kit.js";
import { Rotation, OPENER, INTRO, ECHO_CAST, OUTRO_NEXT } from "../../rotation.js";
import { AERO_EROSION, SPECTRO_FRAZZLE, HAVOC_BANE, FUSION_BURST, GLACIO_CHAFE, ELECTRO_FLARE, HEALS } from "../../statuses.js";
...
const SAND_IN_THE_STORM = new Buff({
  ...
  convertStats: () => { if (casting(Cast.Outro)) revokeSelf(SAND_IN_THE_STORM); },
});
...
const S4_SKILL_BONUS = new Buff({
  ...
  convertStats: () => { if (casting(Cast.Outro)) revokeSelf(S4_SKILL_BONUS); },
});
...
const ROVER_AERO = new Resonator({
  ...
  updateDebuffs: () => {
    ...
    if (a === Cloudburst1 || a === Cloudburst2 || a === UnboundFlow1 || a === UnboundFlow2 || a === Liberation) applySelf(HEALS, 1);
  },
