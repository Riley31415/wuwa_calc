...
export const BLAZING_BRILLIANCE = refinements((r, rank) => {
  ...
  return new Weapon({
    ...
    grants: [
      ...
      { on: onType(Type1.Skill), buff: SEARING_FEATHER, stacks: 5 },
    ],
  });
});
...
export const RED_SPRING = refinements((r, rank) => {
  ...
  return new Weapon({
    ...
    grants: [
      { on: onType(Type1.Basic), buff: RED_SPRING_BASIC, onHit: true },
...
export const UNFLICKERING_VALOR = refinements((r, rank) => {
  ...
  return new Weapon({
    ...
    grants: [
      ...
      { on: onType(Type1.Basic), buff: LAUGHTER_PREVAILS_BASIC, onHit: true },
    ],
  });
});
...
export const EMERALD_SENTENCE = refinements((r, rank) => {
  ...
  const BAMBOO_CLEAVER: Buff = new Buff({
    name: `Emerald Sentence: Bamboo Cleaver${rank}`, maxStacks: 2, duration: 60 * 12, until: LifeTime.Swap,
...
export const FROSTBURN = refinements((r, rank) => {
  ...
  const SELF_NO_MORE_CHAFE = new Buff({
    name: `Frostburn: Self No More${rank} (chafe)`, until: LifeTime.Outro, duration: 60 * 6,
    stats: [[Stat.Amp, [20, 25, 30, 35, 40][r]!, Type2.GlacioChafe]], when: isActive,
  });
  return new Weapon({
