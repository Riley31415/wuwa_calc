...
import {
  add, grantSelf, grantGlobal, grantOthers, revoke, removeStack, stacksOf, outro, isOutro,
} from "../state.js";
...
export const ACTION_BELL_BORNE = new Action("Echo: Bell-Borne Geochelone", {
  ...
  apply() { grantGlobal(BELL_BORNE_SHIELD, 2); },
});

export const BELL_BORNE_SHIELD = new GlobalBuff(PRIORITY.BUFF_STATS, (stacks, a) => {
  add(10, DmgBonus);
  if (isOutro(a)) removeStack(BELL_BORNE_SHIELD, 1);
...
export const HERON = new Gear((stacks, a) => {
  if (isOutro(a) && stacksOf(HERON_PRIMED)) { outro(HERON_HANDOFF); revoke(HERON_PRIMED); }
  return "Impermanence Heron";
});

export const HERON_PRIMED = new Buff(PRIORITY.BUFF_STATS, (stacks, a) => {
  if (isOutro(a)) revoke(HERON_PRIMED);
  return "Impermanence Heron";
});

export const HERON_HANDOFF = new Buff(PRIORITY.BUFF_STATS, (stacks, a) => {
  if (isOutro(a)) revoke(HERON_HANDOFF);
  add(12, DmgBonus);
...
export const ACTION_HERON = new Action("Echo: Heron", {
  cast: Echo, element: Havoc, scaling: Atk, type: Echo,
  mv: 310.56, energy: 4.85,
  priority: PRIORITY.UPDATE_BUFFS,
  apply() { grantSelf(HERON_PRIMED); },
});
...
export const MOONLIT_CLOUDS_2PC = new Gear(() => { add(10, Er); return "Moonlit Clouds 2pc"; });

export const MOONLIT_CLOUDS_5PC = new Gear((stacks, a) => {
  if (isOutro(a)) outro(MOONLIT_CLOUDS_HANDOFF);
  return "Moonlit Clouds 5pc";
});

export const MOONLIT_CLOUDS_HANDOFF = new Buff(PRIORITY.BUFF_STATS, (stacks, a) => {
  add(22.5, BonusAtk);
  if (isOutro(a)) revoke(MOONLIT_CLOUDS_HANDOFF);
...
export const FALLACY_TEAM = new Buff(PRIORITY.BUFF_STATS,
  () => { add(10, BonusAtk); return "Fallacy of No Return"; });
 ...
 *  completeness only. */
export const REJUV_5PC = new Gear(() => { grantGlobal(REJUV_TEAM); return "Rejuvenating Glow 5pc"; });
export const REJUV_TEAM = new GlobalBuff(PRIORITY.BUFF_STATS,
  () => { add(15, BonusAtk); return "Rejuvenating Glow"; });

export const REJUV_2PC = new Gear(() => { add(10, HealingBonus); return "Rejuvenating Glow 2pc"; });

/** Sierra Gale 2pc, a generic sonata — Iuno's aero 2pc: +10% Aero DMG Bonus flat. */
export const SIERRA_2PC = new Gear(() => { add(10, Aero, DmgBonus); return "Sierra Gale 2pc"; });
